...
### 1. Основной скрипт: `backup_ftp.py`
...
class FTPBackup:
    ...
    def _verify_file_integrity(self, ftp: ftplib.FTP, local_file: Path, remote_filename: str) -> bool:
        ...
        try:
            ...
            if local_size != remote_size:
                self.logger.error(f"Size mismatch for {remote_filename}: local={local_size:,} bytes, remote={remote_size:,} bytes")
                return False
    ...
    def _upload_file_to_ftp_with_retry(self, ftp: ftplib.FTP, local_file: Path, remote_filename: str, remote_dir: str, max_retries: int = 3) -> bool:
            ...
            for base_name, file_list in archive_sets.items():
                ...
                try:
                    ...
                    if success:
                        ...
                        self.logger.error(f"Set {processed_count}/{len(archive_sets)}: FAILED - set will not be removed locally")
...
### 2. Утилита проверки статуса: `check_status.py`
...
### 3. Утилита очистки логов: `cleanup_logs.py`
...
### 4. Пример конфигурации: `config.ini.example`
...
### 5. Пример файла исключений: `.ftpignore.example`
...
### 6. Скрипт установки cron: `setup_cron.sh`
...
### 7. Файл зависимостей: `requirements.txt`

```
# FTP Backup dependencies
# No external dependencies required for basic functionality
```

### 8. Git ignore файл: `.gitignore`
...
# Ignore example files (they should be copied)
!config.ini.example
!.ftpignore.example
```
...
cp .ftpignore.example .ftpignore
...
```ini
[FTP]
server = ваш-ftp-сервер.com
username = ваш_логин
password = ваш_пароль
remote_dir = /путь/на/ftp/

[LOCAL]
source_dir = /локальный/путь/к/архивам

[SETTINGS]
max_copies = 5          # Сколько наборов хранить на FTP
upload_retries = 3      # Количество попыток загрузки

[LOGGING]
level = INFO            # Уровень логирования: DEBUG, INFO, WARNING, ERROR
log_file = log/backup.log
max_size_mb = 10        # Максимальный размер лог-файла
backup_count = 10       # Сколько архивных логов хранить
```

### 3. Установка прав

```bash
chmod +x backup_ftp.py check_status.py cleanup_logs.py setup_cron.sh
```

### 4. Тестовый запуск

```bash
python backup_ftp.py
```

### 5. Установка cron задачи
...
Пример: `crm.rusvek29.ru_20260127_020401_full_6mq5xw84z01osj0q.tar`
...
**Примечание:** Перед использованием в production обязательно протестируйте скрипт в тестовой среде с тестовыми данными.